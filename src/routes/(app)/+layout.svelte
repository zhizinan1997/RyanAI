<script lang="ts">
	...
	import { getNotifications, getBanners } from '$lib/apis/configs';
...
<SplashNoticeOverlay
	bind:show={showSplashNotice}
	title={$config?.ui?.splash_notice_title ?? ''}
	content={$config?.ui?.splash_notice_content ?? ''}
	mediaUrl={$config?.ui?.splash_notice_media_url ?? ''}
/>
