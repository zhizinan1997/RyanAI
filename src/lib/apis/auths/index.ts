...
export const uploadSplashNoticeMedia = async (token: string, media: File) => {
	const data = new FormData();
	data.append('media', media);

	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/admin/config/splash-notice/media`, {
		method: 'POST',
		headers: {
...
export const deleteSplashNoticeMedia = async (token: string) => {
	let error = null;
...
export const verifySignupEmail = async (email: string, code: string) => {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/signup_verify`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json'
		},
...
export const resendSignupVerification = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/auths/signup_verify/resend`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
