...
import type { Banner, Notification, NotificationListResponse } from '$lib/types';
 ...
 * Proxied through the RyanAI backend to keep API keys server-side.
 */
...
export const getNotifications = async (
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/notifications?${params.toString()}`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const createNotification = async (
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/notifications`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const getUsageConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/usage`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setUsageConfig = async (token: string, config: object) => {
	let error = null;
