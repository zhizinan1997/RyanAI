				...
				>
					...
					{#if type === 'mcp'}
						...
						>
							...
							{$i18n.t(
								'MCP support is experimental and its specification changes often, which can lead to incompatibilities. OpenAPI specification support is directly maintained by the RyanAI team, making it the more reliable option for compatibility.'
							)}

							<a class="font-medium underline" href="https://github.com/zhizinan1997/RyanAI/tree/main/docs" target="_blank"
								>{$i18n.t('Read more →')}</a
							>
						</div>
					{/if}
