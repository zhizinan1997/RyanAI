<script lang="ts">
	...
	$: adminSettingGroups = {
		...
		'admin:splash-notice': $i18n.t('Experience'),
		'admin:db': $i18n.t('Data'),
		'admin:credit': $i18n.t('Billing')
	};
			...
			{#if filteredPersonalSettings.length > 0}
				{#each filteredPersonalSettings as tabId, index (tabId)}
					...
					{:else if tabId === 'credit'}
						<button
							role="tab"
							aria-controls="tab-credit"
							aria-selected={selectedTab === 'credit'}
							class={tabButtonClass(selectedTab === 'credit')}
							on:click={() => {
								selectedTab = 'credit';
							}}
						>
