...
export class MockAdapter implements MockInjectableAdapter {
	...
	async start(): Promise<void> {
		for (const channel of this.config.enabledChannels) {
			...
			if (!this.state.getConnection(id)) {
	...
	async deleteConnection(connectionId: string): Promise<void> {
		if (!this.state.getConnection(connectionId)) throw new AdapterError('connection_not_found', 404);
	...
	async login(connectionId: string, _input: LoginInput): Promise<ConnectionSnapshot> {
		const channel = this.resolveConnection(connectionId);
		const existing = this.state.getConnection(connectionId);
	...
	async logout(connectionId: string): Promise<ConnectionSnapshot> {
		...
		const snapshot: ConnectionSnapshot = {
			id: connectionId,
			channel,
			enabled: this.state.getConnection(connectionId)?.enabled ?? true,
			status: 'logged_out',
			updatedAt: new Date().toISOString(),
			detail: 'Mock adapter'
		};
	...
	async getQrCode(connectionId: string): Promise<QrCodeSnapshot> {
		this.resolveConnection(connectionId);
	...
	private resolveConnection(connectionId: string): Channel {
		const connection = this.state.getConnection(connectionId);
