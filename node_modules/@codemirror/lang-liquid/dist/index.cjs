...
const base = new lr.ExternalTokenizer(input => {
    let start = input.pos;
    for (;;) {
        let { next } = input;
        if (next < 0)
...
const inlineComment = new lr.ExternalTokenizer(input => {
...
const Filters = completions("abs append at_least at_most capitalize ceil compact concat date default " +
...
const Tags = completions("cycle comment endcomment raw endraw echo increment decrement liquid if elsif " +
...
function findContext(context) {
    ...
    let node = language.syntaxTree(state).resolveInner(pos, -1).enterUnfinishedNodesBefore(pos);
...
cursor between them.
*/
const closePercentBrace = view.EditorView.inputHandler.of((view, from, to, text) => {
...
const tagLanguage = language.LRLanguage.define({
    name: "liquid",
    parser: parser.configure({
        props: [
            highlight.styleTags({
                "cycle comment endcomment raw endraw echo increment decrement liquid in with as": highlight.tags.keyword,
                "empty forloop tablerowloop": highlight.tags.atom,
                "if elsif else endif unless endunless case endcase for endfor tablerow endtablerow break continue": highlight.tags.controlKeyword,
                "assign capture endcapture": highlight.tags.definitionKeyword,
                "contains": highlight.tags.operatorKeyword,
                "render include": highlight.tags.moduleKeyword,
                VariableName: highlight.tags.variableName,
                TagName: highlight.tags.tagName,
                FilterName: highlight.tags.function(highlight.tags.variableName),
                ...
                InlineComment: highlight.tags.lineComment,
                CommentText: highlight.tags.blockComment,
            ...
            language.indentNodeProp.add({
                Tag: language.delimitedIndent({ closing: "%}" }),
...
function makeLiquid(base) {
    return tagLanguage.configure({
        wrap: common.parseMixed(node => node.type.isTop ? {
            parser: base.parser,
...
*/
const liquidLanguage = makeLiquid(baseHTML.language);
/**
...
exports.closePercentBrace = closePercentBrace;
exports.liquid = liquid;
exports.liquidCompletionSource = liquidCompletionSource;
exports.liquidLanguage = liquidLanguage;
