...
class CompletionUsage(BaseModel):
    ...
    def format_input(cls, data: dict) -> dict:
        if not isinstance(data, dict):
            return data
        # standard tokens
        prompt_tokens = data.pop('prompt_tokens', 0) or data.pop('promptTokenCount', 0) or data.pop('input_tokens', 0)
        completion_tokens = (
            data.pop('completion_tokens', 0) or data.pop('candidatesTokenCount', 0) or data.pop('output_tokens', 0)
        )
        total_tokens = (
            data.pop('total_tokens', 0) or data.pop('totalTokenCount', 0) or (prompt_tokens + completion_tokens)
        )
        # update data
        data.update(
            {
...
class Choice(BaseModel):
    model_config = ConfigDict(extra='allow')
    message: Optional[ChatCompletionMessage] = Field(default_factory=lambda: ChatCompletionMessage())
