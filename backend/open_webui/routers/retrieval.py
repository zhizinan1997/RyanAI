...
async def update_embedding_config(request: Request, form_data: EmbeddingModelUpdateForm, user=Depends(get_admin_user)):
    ...
    try:
        ...
        if DISABLE_LOCAL_EMBEDDING_MODELS and config.RAG_EMBEDDING_ENGINE == '':
            log.info(
                'Local embedding models are disabled; forcing RAG_EMBEDDING_ENGINE to openai '
                'to avoid Hugging Face model downloads.'
            )
            config.RAG_EMBEDDING_ENGINE = 'openai'
            if not config.RAG_EMBEDDING_MODEL or config.RAG_EMBEDDING_MODEL.startswith('sentence-transformers/'):
                config.RAG_EMBEDDING_MODEL = os.getenv('RAG_EMBEDDING_MODEL', 'text-embedding-3-small')
