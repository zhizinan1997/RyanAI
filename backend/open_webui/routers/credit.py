...
async def create_ticket(
    ...
    return TradeTickets.insert_new_ticket(id=out_trade_no, user_id=user.id, amount=form_data['amount'], detail=detail)
...
async def get_statistics(form_data: StatisticRequest, _: UserModel = Depends(get_admin_user)):
    ...
    trade_logs = TradeTickets.get_ticket_by_time(form_data.start_time, form_data.end_time, user_ids)
...
async def get_redemption_codes(
    ...
    if not codes:
        return {'total': 0, 'results': []}
    # query users
    code_user_ids = [code.user_id for code in codes if code.user_id]
    users = await Users.get_users_by_user_ids(user_ids=code_user_ids)
    user_map = {user.id: user.name for user in users}
    for code in codes:
...
async def create_redemption_code(form_data: CreateRedemptionCodeForm, _: UserModel = Depends(get_admin_user)) -> dict:
    """
...
async def delete_redemption_codes(code: str, _: UserModel = Depends(get_admin_user)) -> None:
    """
...
async def export_redemption_codes(keyword: str, _: UserModel = Depends(get_admin_user)) -> Response:
    """
        ...
        csv_content += (
            ','.join(
                [
                    ...
                    datetime.datetime.fromtimestamp(code.created_at).strftime('%Y-%m-%d %H:%M:%S'),
                    (
                        datetime.datetime.fromtimestamp(code.expired_at).strftime('%Y-%m-%d %H:%M:%S')
                        if code.expired_at
                        else ''
                    ),
                    (
                        datetime.datetime.fromtimestamp(code.received_at).strftime('%Y-%m-%d %H:%M:%S')
                        if code.received_at
                        else ''
                    ),
                ]
...
async def receive_redemption_code(code: str, user: UserModel = Depends(get_verified_user)) -> None:
    """
