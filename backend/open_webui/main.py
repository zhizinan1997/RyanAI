...
RyanAI v{VERSION} - building the best AI user interface.
...
"""
    try:
        ...
        print(f'RyanAI v{VERSION} - building the best AI user interface.\nhttps://github.com/zhizinan1997/RyanAI')
...
if DISABLE_LOCAL_EMBEDDING_MODELS and app.state.config.RAG_EMBEDDING_ENGINE == '':
    app.state.config.RAG_EMBEDDING_ENGINE = 'openai'
    if (
        not app.state.config.RAG_EMBEDDING_MODEL
        or app.state.config.RAG_EMBEDDING_MODEL.startswith('sentence-transformers/')
    ):
        app.state.config.RAG_EMBEDDING_MODEL = os.getenv('RAG_EMBEDDING_MODEL', 'text-embedding-3-small')
...
app.state.config.ENABLE_DAILY_CHECKIN = ENABLE_DAILY_CHECKIN
...
async def generate_messages(
    ...
    All models configured in RyanAI are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/zhizinan1997/RyanAI/releases/latest',
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for RyanAI.
