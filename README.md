...
| 积分计费          | 支持模型调用、Embedding、图片生成、代码执行、网页搜索和工具调用计费，并记录 Token 用量。   |
| 自定义定价        | 支持模型价格、最低消费、初始积分、特性价格，以及基于请求 Body 的 JSONPath 自定义计费规则。 |
| 积分管理          | 支持初始积分、管理员调整余额及消费记录查询。                                               |
| 用户运营          | 支持邮箱验证注册、邮箱域名白名单、用户积分与消费记录管理。                                 |
| 数据报表          | 提供积分消耗、Token 使用、积分流水、模型消耗和用户维度统计。                               |
...
更多配置可以在管理员面板中完成，包括模型接入、积分、SMTP、用户权限、工作区和工具服务。
...
RyanAI 会在尊重上游贡献和许可证要求的前提下，继续按照自己的产品定位演进：更关注中文运营场景、积分计费、用户管理和团队可落地部署。
